...
import { localizeNewsArticle } from './translator.js';
...
export async function fetchGdeltNews(ticker, query, days = 7) {
  const q = isCryptoTicker(ticker)
    ? `("${query}" OR "${ticker}") (crypto OR bitcoin OR ethereum OR ETF OR funding OR liquidation OR exchange OR whale OR on-chain OR macro)`
    : `("${query}" OR "${ticker}") (stock OR shares OR nasdaq OR nyse OR amex OR earnings OR contract OR offering OR sec)`;
...
export async function fetchGoogleNewsRss(ticker, query, days = 7) {
  const q = isCryptoTicker(ticker)
    ? `${query} ${ticker} crypto bitcoin ETF funding liquidation when:${Math.max(1, Math.min(days, 30))}d`
    : `${query} ${ticker} stock when:${Math.max(1, Math.min(days, 30))}d`;
...
export function rankAndCleanNews(articles, ticker, days = 7) {
  ...
  return (articles || [])
    ...
    .map(a => localizeNewsArticle(a, ticker));
}
...
function fallbackNews(ticker, query) {
  if (isCryptoTicker(ticker)) {
    return rankAndCleanNews([
      {
        title: `${query}: เปิดลิงก์ค้นข่าวคริปโตจริงจาก Google News / Yahoo Finance เพื่อยืนยัน catalyst ล่าสุด`,
        url: `https://www.google.com/search?q=${encodeURIComponent(`${query} crypto news last 7 days`)}`,
      ...
      {
        title: `${query}: จับตา ETF flow, Fed, DXY, funding rate, open interest, exchange inflow/outflow และ whale movement`,
        url: `https://finance.yahoo.com/quote/${ticker}/news`,
        source: 'Fallback guidance',
        publishedAt: new Date().toISOString(),
        snippet: 'ปัจจัยเหล่านี้มีผลต่อราคา Bitcoin/Crypto ในวันถัดไป โดยเฉพาะช่วงตลาดผันผวนสูงและมี leverage crowded',
        origin: 'fallback'
      }
    ], ticker, 7);
  }
