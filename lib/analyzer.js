...
import { buildCryptoAgentAnalyses, buildCryptoFactors, buildCryptoSummary, cryptoExternalLinks, cryptoProfileForSymbol, cryptoTradingViewSymbol, isCryptoMode, normalizeCryptoSymbol } from './crypto.js';
...
export async function analyzeSymbol({ symbol = 'BURU', market = 'AMEX', days = 90, newsDays = 7, assetType = 'stock' } = {}) {
  const cryptoMode = isCryptoMode(assetType, market, symbol);
  const ticker = cryptoMode ? normalizeCryptoSymbol(symbol || 'BTC') : normalizeTicker(symbol);
  const effectiveMarket = cryptoMode ? 'CRYPTO' : String(market || 'AMEX').toUpperCase();
  ...
  const stockFactors = buildFactors(ticker, prediction, newsResult.articles, socialResult, smartMoney);
  const factors = cryptoMode
    ? buildCryptoFactors(ticker, prediction, newsResult.articles, socialResult, smartMoney)
    : stockFactors;
  const profile = cryptoMode ? cryptoProfileForSymbol(ticker) : profileForSymbol(ticker);
  const summary = cryptoMode
    ? buildCryptoSummary(prediction, newsResult.fallback, historyResult.source)
    : buildSummary(prediction, factors, newsResult.fallback, historyResult.source);
  const agents = cryptoMode
    ? buildCryptoAgentAnalyses({
        symbol: ticker,
  ...
  return {
    symbol: ticker,
    assetType: cryptoMode ? 'crypto' : 'stock',
    assetLabel: cryptoMode ? 'Bitcoin / Crypto' : 'หุ้นอเมริกา',
    market: effectiveMarket,
    tradingViewSymbol: cryptoMode ? cryptoTradingViewSymbol(ticker) : marketSymbol(effectiveMarket, ticker),
    ...
    smartMoney,
    agents,
    externalLinks: cryptoMode ? cryptoExternalLinks(ticker, newsResult.query) : externalLinks(ticker, newsResult.query)
  };
}
