...
export function buildFactors(symbol, prediction, articles = []) {
  ...
  const newsLabel = prediction.newsSentiment > 0.2 ? 'Bullish' : prediction.newsSentiment < -0.2 ? 'Bearish' : 'Neutral';

  return [
    ['News', newsLabel, `คะแนนข่าวรวม ${prediction.newsSentiment} จาก -1 ถึง +1 โดยถ่วงน้ำหนักตาม impact ของข่าว`],
    ['Situation', prediction.verdict, `ราคา ${formatMoney(tech.last)} เทียบ MA20 ${formatMoney(tech.ma20)} และ VWAP ${formatMoney(tech.vwap20)}`],
    ['Online Sentiment', articles.length ? 'Tracked' : 'Limited', `ใช้ headline sentiment จากข่าว ${articles.length} รายการ`],
    ['Whale / Big Money', tech.volRatio > 1.4 ? 'Active' : 'Quiet', `Volume ล่าสุด ${tech.volRatio}x ของค่าเฉลี่ย 20 วัน`],
    ['Volume', tech.volRatio > 1.2 ? 'High' : tech.volRatio < 0.75 ? 'Low' : 'Normal', `Volume ratio = ${tech.volRatio}`],
    ['Koncorde', 'Not Connected', 'เตรียมช่องต่อ indicator เฉพาะทางภายหลัง'],
    ['MACD', tech.macd.histogram > 0 ? 'Positive' : 'Negative', `Histogram = ${tech.macd.histogram}`],
    ['RSI', tech.rsi14 > 70 ? 'Overbought' : tech.rsi14 < 30 ? 'Oversold' : 'Neutral', `RSI14 = ${tech.rsi14}`],
    ['Risk / ATR', tech.atrPct > 0.06 ? 'High Risk' : 'Normal', `ATR = ${(tech.atrPct * 100).toFixed(2)}% ของราคา`],
    ['Trend Structure', tech.trendScore > 0.2 ? 'Improving' : tech.trendScore < -0.2 ? 'Weak' : 'Sideways', `Trend score = ${tech.trendScore}`],
    ['VWAP', tech.last > tech.vwap20 ? 'Above VWAP' : 'Below VWAP', `ราคาปัจจุบัน ${tech.last > tech.vwap20 ? 'อยู่เหนือ' : 'อยู่ใต้'} VWAP20`]
